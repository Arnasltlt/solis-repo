-- Drop existing content_feedback table
DROP TABLE IF EXISTS content_feedback;

-- Create content_feedback table with rating
...
CREATE OR REPLACE FUNCTION get_content_feedback_count(content_id UUID, rating INTEGER)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
BEGIN
...
COMMENT ON TABLE content_feedback IS 'Stores user feedback (likes/dislikes) for content';
COMMENT ON COLUMN content_feedback.rating IS 'Rating value: -1 for dislike, 1 for like'; 